...
def upload_thread(temp_year, temp_month) :
    ...
    while True:                                 #무한 반복
        ...
        if os.path.getsize(checkSize) == temp :
            ...
            if now.year == temp_year :      #암호화된 영상 파일을 IPFS 디렉토리에 저장 후 갱신하기 위한 조건문 시작, 년도/월 구분
                if now.month == temp_month :
                    root_hash = updateDir.dirUpdate1(now.year, now.month, ipfsAdd)
                elif now.month != temp_month :
                    temp_month += 1 #기존에 저장된 temp_month와 현재의 now.month가 다르다면 한 달이 넘어갔음을 의미하므로 temp_month를 +1 해줌
                    root_hash = updateDir.dirUpdate2(now.year, now.month, ipfsAdd)
            elif now.year != temp_year and now.month == 1 : #년도가 바뀌었을 떄
                temp_year += 1
                temp_month = 1
