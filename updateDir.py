...
def dirUpdate1(temp_year, temp_month, ipfsAd) :         #년도/월이 이전과 같을 때
    ...
    if 'aCAM' in clip_name :                            #영상 이름이 aCAM이라면 조건문 실행
        ...
        up_m = subprocess.check_output('/usr/local/bin/ipfs object patch '+ month_hash +' add-link ' + clip_name + ' ' + clip_hash, universal_newlines=True, stderr=subprocess.STDOUT, shell=True).strip() #기존의 IPFS directory에 month에 해당하는 위치에 영상을 link했을 때 반환되는 수정된 month의 hash를 저장
...
def dirUpdate3(temp_year, temp_month, ipfsAd) :
    ...
    if 'aCAM' in clip_name :
        ...
        insert_value = ('rootDir/'+str(temp_year), str(emptyDir))   #년도가 다르다면 년도, 월(1) 두개의 디렉토리를 생성해야 하므로 먼저 데이터베이스에 INSERT 해줌
        cur.execute(sql, insert_value)
        conn.commit()
        insert_value = ('rootDir/'+str(temp_year)+'/'+str(temp_month), str(emptyDir))
        cur.execute(sql, insert_value)
