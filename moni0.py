...
def Camera(i) :
    ...
    try:
        sub = subprocess.check_output('openRTSP -D 1 -c -B 10000000 -b 10000000 -q -Q -F aCAM'+ str(i) +' -d 28800 -P 60 -t -u admin admin rtsp://192.168.1.26/11', stderr=subprocess.STDOUT, shell=True)
    except :
        print("error")
...
def upload_thread(temp_year, temp_month) :
    ...
    while True:                                 #무한 반복
        ...
        if os.path.getsize(checkSize) == temp :
            ...
            elif now.year != temp_year and now.month == 1 :
                temp_year += 1
                temp_month = 1
                root_hash = dirUpdate3(now.year, now.month, ipfsAdd)
